...
function SearchPage() {
  ...
  useEffect(()=>{
    ...
    if (filter.type.length>0 && datarender.length>0){
        setDataRender(datarender.filter((e)=>{
            return filter.type.indexOf(e.type)!= -1
        }));
    }
    if (filter.neigbor.length>0 && datarender.length>0){
        setDataRender(datarender.filter((e)=>{
            return filter.neigbor.indexOf(e.district)!= -1
        }));
    }
    if (filter.exten.length>0&& datarender.length>0) {
       let data=[]
       for (const element of filter.exten) {
        console.log(datarender);
        data=[...data,...datarender.filter((e)=>{
